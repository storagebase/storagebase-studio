...
| Kafka workbench in the Connections list | `src/components/sidebar/ConnectionsList.tsx` (optional `trailingItems` slot), `src/components/sidebar/Sidebar.tsx` (workbench props), `src/components/Studio.tsx` (mounts the workbench over `<main>`) — all additive; Kafka itself stays a resource type (`src/lib/resources/**`), routed by `opensWorkbench()` in `src/lib/resources/ui-config.ts` | Kafka is managed like a connection (kafbat-style workbench) rather than a small resource tree | fork-only |
