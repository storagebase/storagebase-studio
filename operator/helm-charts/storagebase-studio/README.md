# StorageBase Studio Helm Chart

[![Artifact Hub](https://img.shields.io/endpoint?url=https://artifacthub.io/badge/repository/storagebase-studio)](https://artifacthub.io/packages/search?repo=storagebase-studio)
...
Deploys a Bitnami PostgreSQL instance alongside StorageBase Studio.
...
helm install storagebase storagebase/storagebase-studio \
  ...
  --set secrets.storagePostgresUrl="postgresql://user:pass@host:5432/storagebase" \
  --set secrets.jwtSecret=$(openssl rand -base64 32) \
  --set secrets.adminPassword=MyAdmin123
```
...
kubectl logs deployment/storagebase-storagebase-studio --previous | grep -A 4 "generated admin credentials"

# 3. Or read the file the app stored them in (mode 0600, survives restarts)
kubectl exec deploy/storagebase-storagebase-studio -- cat /app/data/auth-bootstrap.json
```
...
```bash
helm upgrade storagebase storagebase/storagebase-studio --reuse-values --set agent.enabled=false
```
...
```bash
helm upgrade storagebase storagebase/storagebase-studio --reuse-values --set agent.threadContext=false
```
...
helm upgrade storagebase storagebase/storagebase-studio --reuse-values \
...
```bash
kubectl rollout restart deployment/storagebase-storagebase-studio
```
...
helm install storagebase storagebase/storagebase-studio \
  ...
  --set-string 'extraEnv[0].value=https://storagebase.example.com'
```
...
helm install storagebase storagebase/storagebase-studio \
  ...
  --set 'extraEnv[0].valueFrom.secretKeyRef.name=storagebase-studio-storage' \
...
[docs/STORAGE.md](https://github.com/storagebase/storagebase-studio/blob/main/docs/STORAGE.md#credential-encryption-at-rest).
...
```bash
helm install storagebase storagebase/storagebase-studio \
  --set secrets.existingSecret=my-storagebase-secret
```
...
```bash
helm repo update
helm upgrade storagebase storagebase/storagebase-studio
```
...
```bash
helm uninstall storagebase
```

> **Note:** PVCs are not deleted automatically. To remove persistent data:
> ```bash
> kubectl delete pvc -l app.kubernetes.io/instance=storagebase
> ```
...
| `secrets.adminEmail` | Admin email | `admin@storagebase.org` |
| `secrets.adminPassword` | Admin password | `""` |
| `secrets.userEmail` | User email | `user@storagebase.org` |
...
Build a custom image with `docker build --build-arg BASE_PATH=/tools/storagebase ...`, then set
`config.basePath: /tools/storagebase` and the matching image repository/tag. This prefixes the default
...
See [subpath deployment](https://github.com/storagebase/storagebase-studio/blob/main/docs/SUBPATH.md)
